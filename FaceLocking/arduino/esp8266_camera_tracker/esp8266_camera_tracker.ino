/*
 * BENAX Technologies - AI Speaker Tracking System
 * ESP8266 Servo Controller (Assessment)
 * 
 * Receives MQTT motion commands from the Python AI pipeline
 * and drives a servo motor to keep the enrolled speaker centered.
 * 
 * Hardware (as per assessment spec):
 * - ESP8266 NodeMCU
 * - SG90 Servo motor (horizontal pan)
 * - FalkonEye V1 HD Camera on 2-DOF tilt mount
 * 
 * Wiring (as physically connected):
 * - Servo Signal -> D4 (GPIO2)
 * - Servo VCC   -> 5V
 * - Servo GND   -> GND
 * 
 * MQTT Topics:
 * - camera/track/horizontal  <-  Receives angle 0-180
 * - camera/track/command     <-  Receives left/right/center
 * - camera/status            ->  Publishes servo position + motion status
 */
...
const char* mqtt_server = "157.173.101.159";  // Teacher's MQTT broker
...
// Servo settings
const int SERVO_PIN = 2;  // D4 = GPIO2 (as wired)
...
void setup() {
  ...
  Serial.println("\n\n=================================");
  Serial.println("BENAX Speaker Tracker Starting");
  Serial.println("=================================\n");
  ...
  Serial.println("[CENTERED] Servo initialized at center position");
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Handle movement commands (left, right, center)
    ...
    if (command == "left" || command == "move_left") {
      ...
      Serial.print("[MOVED_LEFT] Target angle: ");
      Serial.println(targetAngle);
    } else if (command == "right" || command == "move_right") {
      targetAngle = constrain(currentAngle + SERVO_STEP_SIZE, SERVO_MIN_ANGLE, SERVO_MAX_ANGLE);
      Serial.print("[MOVED_RIGHT] Target angle: ");
      Serial.println(targetAngle);
    } else if (command == "center") {
      targetAngle = SERVO_CENTER_ANGLE;
      Serial.print("[CENTERED] Target angle: ");
      Serial.println(targetAngle);
    } else {
...
void updateServoPosition() {
  ...
  if (currentAngle != targetAngle) {
    ...
    // Movement completed
    if (currentAngle == targetAngle) {
      Serial.println("[STOPPED] Movement complete");
