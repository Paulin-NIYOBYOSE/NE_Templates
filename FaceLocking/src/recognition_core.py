...
def load_database() -> Dict[str, np.ndarray]:
    if not config.DB_NPZ_PATH.exists():
        print("  ✗ Database not found. Run: python -m src.enroll")
        return {}
...
def choose_lock_identity(names: list) -> Optional[str]:
    ...
    for i, n in enumerate(names, 1):
        print(f"    {i}. {n}")
    print("\n  Select speaker to lock (number/name, or Enter to skip): ", end="")
    try:
        raw = input().strip()
    except EOFError:
        return None
...
def draw_results(vis: np.ndarray, results: List[FaceResult], tracking_state: str = "") -> None:
    """Draw face boxes with color-coded identity status."""
    for result in results:
        face = result.face
        ...
        if result.is_locked_person:
            ...
            lines = [f"[SPEAKER] {result.name}", f"Match: {conf_pct}%"]
        elif result.accepted:
            color = config.COLOR_KNOWN
            lines = [result.name, f"Match: {conf_pct}%"]
        else:
...
def draw_tracks(
    ...
    """
    Draw tracked faces with color-coded identity status.

    - Unregistered       -> RED
    - Known, not locked  -> BLUE  "Name" + "Match: %"
    - Locked & visible   -> GREEN "[SPEAKER] Name" + "Match: %"
    """
    mid_x = vis.shape[1] // 2
    for tr in tracks:
        bbox = tr.bbox
        ...
        if is_locked:
            color = config.COLOR_LOCKED
            lines = [f"[SPEAKER] {tr.name}", f"Match: {conf_pct}%"]
            thickness = 3
        elif tr.accepted:
            color = config.COLOR_KNOWN
            lines = [f"{tr.name}", f"Match: {conf_pct}%"]
            thickness = 2
        else:
            color = config.COLOR_UNKNOWN
            lines = ["Unregistered"]
