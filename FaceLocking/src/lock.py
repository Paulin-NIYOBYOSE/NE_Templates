"""
FaceLock Vision - Speaker Lock Module
Developed by Paulin | Speaker locking with action recording
"""
...
def main():
    """Speaker Locking: select one identity, lock onto that speaker, track and record actions."""
    db = load_database()
    if not db:
    ...
    for i, n in enumerate(names, 1):
        print(f"    {i}. {n}")
    print("\n  Enter speaker name to lock: ", end="")
    try:
        choice = input().strip()
    except EOFError:
    ...
    if not choice:
        choice = names[0] if names else ""
    if choice not in db:
        print(f"  ✗ '{choice}' not found. Options: {names}")
        return False
    lock_identity = choice
    print(f"  ● Speaker lock armed: {lock_identity}")
    ...
    print("\n  Speaker tracking active — face appears → auto-lock  |  [Q]uit")
    ...
    try:
        while True:
            ...
            if not locked:
                for face in faces:
                    ...
                    if best_idx == lock_idx and best_dist <= threshold:
                        ...
                        mouth_width_samples = []
                        baseline_mouth_width = None
                        print(f"  ● SPEAKER LOCKED: {lock_identity}  |  Log: {history_path.name}")
                        break
                ...
                if matched_face is None:
                    ...
                    if fail_count >= config.LOCK_RELEASE_FRAMES:
                        ...
                        if history_file:
                            history_file.close()
                            history_file = None
                        print(f"  ● Lock released (lost for {config.LOCK_RELEASE_FRAMES} frames)")
                else:
                    ...
                    cv2.putText(vis, "[SPEAKER] " + lock_identity, (matched_face.x1, max(0, matched_face.y1 - 15)),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                    cv2.putText(vis, "match=%.3f" % best_dist, (matched_face.x1, matched_face.y2 + 25),
            ...
            if locked:
                for face in faces:
                    ...
                    cv2.putText(vis, "Other face", (face.x1, face.y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 1)
            else:
                for face in faces:
                    cv2.rectangle(vis, (face.x1, face.y1), (face.x2, face.y2), (0, 255, 0), 2)
                    cv2.putText(vis, "Searching speaker...", (face.x1, face.y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

            header = "Speaker: " + lock_identity + " | " + ("LOCKED" if locked else "Searching...") + " | %.0f FPS" % fps
            cv2.putText(vis, header, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(vis, "[Q]uit", (10, vis.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
            cv2.imshow("FaceLock - Speaker Track", vis)
        ...
        if history_file:
            history_file.close()
        if history_path:
            print(f"  ● Log saved: {history_path}")
