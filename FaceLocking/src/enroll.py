"""
FaceLock Vision - Enrollment Module
Developed by Paulin | Face registration and template storage
"""
...
def main():
    ...
    print(f"\n  ╔{'═'*44}╗")
    print(f"  ║  FaceLock Vision • Enrollment             ║")
    print(f"  ╚{'═'*44}╝")
    ...
    if list(person_dir.glob("*.jpg")):
        print(f"  ● Found existing registration for {name}. Loading samples...")
    ...
    cv2.namedWindow("FaceLock - Register")
    cv2.namedWindow("FaceLock - Face Preview")
    cv2.resizeWindow("FaceLock - Face Preview", 200, 200)
    ...
    try:
        while True:
            ...
            if faces:
                ...
                cv2.imshow("FaceLock - Face Preview", aligned_vis)
            ...
            cv2.imshow("FaceLock - Register", vis)
            cv2.imshow("FaceLock - Face Preview", aligned_vis)
            ...
            if key == ord("q"):
                ...
                print(f"\n  ● Registered '{name}' successfully ({total} samples)")
                return True
