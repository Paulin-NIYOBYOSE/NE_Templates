"""
FaceLock Vision - Threshold Tuner
Developed by Paulin | Analyzes match quality and recommends sensitivity settings
"""
...
def evaluate():
    ...
    if not people_data:
        print("  ✗ No enrollment samples found. Run: python -m src.enroll")
        return False
    
    print(f"\n  Analyzing {len(people_data)} registered faces...")
    ...
    print(f"\n  Same-person matches: {stats_str(genuine)}")
    print(f"  Cross-person matches: {stats_str(impostor)}")
    
    # Threshold sweep
    print("\n  " + "─"*56)
    print("  SENSITIVITY SWEEP")
    print("  " + "─"*56)
    print("\n  Distance | False Accept | False Reject")
    print("  " + "-" * 38)
    ...
    if best_threshold is not None:
        print(f"  ● RECOMMENDED SENSITIVITY: {best_threshold:.2f}")
        print(f"    False Accept Rate: {best_far:.2f}% (target: {config.TARGET_FAR*100:.2f}%)")
        print(f"    False Reject Rate: {best_frr:.2f}%")
        print(f"    Match confidence: {1.0 - best_threshold:.3f}")
    else:
        print(f"  ⚠ No setting met target ({config.TARGET_FAR*100:.2f}%)")
        print("    Try:")
        print("      • Register more samples")
        print("      • Better lighting")
        print("      • Relax the target rate")
