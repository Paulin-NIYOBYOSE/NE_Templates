"""
FaceLock Vision - Activity Monitor (Assessment Version)
Developed by Paulin | Tracks and logs speaker behaviors, confidence, and motor commands
"""
...
class ActivityLogger:
    ...
    def __init__(self, person_name: str, log_dir: Path):
        """
        Initialize activity logger for a specific speaker.
        
        Args:
            person_name: Name of the enrolled speaker
        ...
        with open(self.csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "timestamp", "frame_number", "speaker_identity", "confidence_score",
                "activity_type", "motor_command",
        ...
        print(f"  ● Activity monitor active for: {person_name}")
    ...
    def log_activity(
        ...
        """
        Log an activity to CSV file (assessment format).
        ...
        Args:
            ...
            motor_command: Published motor command (MOVED_LEFT, MOVED_RIGHT, etc.)
        """
        ...
        cmd_str = motor_command if motor_command else ""
        
        with open(self.csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                timestamp, frame_number, self.person_name, conf_str,
                activity_type, cmd_str,
        ...
        if abs(dx) > self.movement_threshold_x:
            if dx > 0:  # Moved right
                if frame_number - self.last_movement_frame.get("move_right", -999) >= self.movement_cooldown:
                    ...
                    print(f"    →  Speaker moved RIGHT ({dx:.1f}px)")
            else:  # Moved left
                if frame_number - self.last_movement_frame.get("move_left", -999) >= self.movement_cooldown:
                    ...
                    print(f"    ←  Speaker moved LEFT  ({dx:.1f}px)")
        ...
        if abs(dy) > self.movement_threshold_y:
            if dy > 0:  # Moved down
                if frame_number - self.last_movement_frame.get("move_down", -999) >= self.movement_cooldown:
                    ...
                    print(f"    ⬇  Speaker moved DOWN  ({dy:.1f}px)")
            else:  # Moved up
                if frame_number - self.last_movement_frame.get("move_up", -999) >= self.movement_cooldown:
                    ...
                    print(f"    ↑  Speaker moved UP    ({dy:.1f}px)")
